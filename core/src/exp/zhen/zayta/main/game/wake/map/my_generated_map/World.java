...
import exp.zhen.zayta.main.game.wake.map.my_generated_map.models.Tile;
import exp.zhen.zayta.main.game.wake.map.my_generated_map.models.TileSprite;
...
//	protected Heap heap;
    ...
    private void setUpGraphics(TextureAtlas mapGeneratorAtlas) {
        ...
        GeneratedCell.setSprites(mapGeneratorAtlas);
    }

	public void initiateWorld() {
		GeneratedCell.setWorld(this);
...
//		heap = new Heap();
	...
	protected void setupTiles() {
		for (GeneratedCell[] pS : getMap()) {
			for (GeneratedCell p : pS) {
				p.setupTile();
			}
		}
	}

	public GeneratedCell[][] getMap() {
		return map;
	}
	public void setMap(GeneratedCell[][] map) {
		this.map = map;
	}

	public GeneratedCell getPair(int x, int y) {
		return map[x][y];
	}
	...
	public Color getColorFromId(Integer level) {
		if (level == GeneratedCell.WATERLEVEL) {
...
//	}
	...
	public boolean walkable(int x, int y){
		...
		GeneratedCell generatedCell = getMap()[x][y];
		if (generatedCell.isWater() || generatedCell.isBorder()) {
			return false;
		}
...
//	 */
//	public int[][] getNeighbors(GeneratedCell node){
