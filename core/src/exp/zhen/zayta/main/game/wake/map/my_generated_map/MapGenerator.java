...
import exp.zhen.zayta.main.game.wake.map.my_generated_map.noise.SimplexNoise;
import exp.zhen.zayta.util.GdxUtils;

public class MapGenerator {

    private static final Logger log = new Logger(MapGenerator.class.getName(),Logger.DEBUG);
	...
	public MapGenerator(int worldWidth, int worldHeight,TextureAtlas mapGeneratorAtlas, TiledMap tileStorage)
	{
		this.worldWidth = worldWidth; this.worldHeight = worldHeight; this.mapGeneratorAtlas=mapGeneratorAtlas;
...
//        TiledMapTileSet cityTileSet = tileStorage.getTileSets().getTileSet("generated_map_tiles_compact");

        TiledMapTileSet cityTileSet = tileStorage.getTileSets().getTileSet("collision_tile_set");
        log.debug("cityTileSet size is "+cityTileSet.size());


        MapLayers layers = tiledMap.getLayers();
        TiledMapTileLayer layer1 = new TiledMapTileLayer(worldWidth,worldHeight,16,16);

        for(int i = 0; i<worldWidth;i++){
            for(int j = 0; j<worldHeight;j++){
                TiledMapTileLayer.Cell cell = new TiledMapTileLayer.Cell();
                cell.setTile(getRandomTile(cityTileSet));
...
//	public TiledMap generateWorld(){
////		World world = new World(worldWidth,worldHeight,mapGeneratorAtlas);
//		TiledMap tiledMap = new TiledMap();
//		tiledMap.getProperties().put("width",worldWidth);
//        tiledMap.getProperties().put("height",worldHeight);
//
//        tiledMap.getTileSets().addTileSet(tileStorage.getTileSets().getTileSet());
//
//		MapLayers layers = tiledMap.getLayers();
//		TiledMapTileLayer layer1 = new TiledMapTileLayer(worldWidth,worldHeight,64,64);
//
//		for(int i = 0; i<worldWidth;i++){
//			for(int j = 0; j<worldHeight;j++){
//				TiledMapTileLayer.Cell cell = new TiledMapTileLayer.Cell();
//				cell.setTile(new StaticTiledMapTile(getRandomRegion()));
//				layer1.setCell(i,j,cell);
//			}
//		}
//
