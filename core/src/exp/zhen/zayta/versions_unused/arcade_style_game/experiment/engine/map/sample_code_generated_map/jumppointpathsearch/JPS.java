package exp.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.map.sample_code_generated_map.jumppointpathsearch;

import java.util.ArrayList;

import exp.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.map.sample_code_generated_map.World;
import exp.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.map.sample_code_generated_map.models.Pair;
...
public class JPS {
    ...
    int[][] neighbors;
    float ng;
    exp.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.map.sample_code_generated_map.models.Pair tmpPair, cur;
    exp.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.map.sample_code_generated_map.models.Pair[] successors, possibleSuccess;
...
//     *
//     */
//    public JPS(World world, GeneratedCell start, GeneratedCell end){
//        this.grid = world;
...
//     */
//    public List<GeneratedCell> search(){
//        grid.getPair(startX,startY).updateGHFP(0, 0, null);
//        grid.heapAdd(grid.getPair(startX, startY));  //Start GeneratedCell is added to the heap
//        while (true){
//            cur = grid.heapPopPair();              //the current GeneratedCell is removed from the heap.
//            if (cur.x == endX && cur.y==endY){		//if the end GeneratedCell is found
...
//                if (trail.size()>0) {
//                    GeneratedCell startPair = grid.getPair(startX, startY);
//                    GeneratedCell endPair = grid.getPair(endX, endY);
//
//                    // For some reason adding a reference to the pair wont work, so we make a copy.
//                    trail.add(0,new GeneratedCell(startPair.x, startPair.y, !startPair.isWater()));
//                    trail.add(new GeneratedCell(endPair.x, endPair.y, !endPair.isWater()));
...
//                break;				//loop is done
//            }
//            possibleSuccess = identifySuccessors(cur);  //get all possible successors of the current GeneratedCell
...
//
//    /**
//     * returns all Pairs jumped from given GeneratedCell
//     *
//     * @param GeneratedCell
//     * @return all Pairs jumped from given GeneratedCell
//     */
//    public GeneratedCell[] identifySuccessors(GeneratedCell GeneratedCell){
//        successors = new GeneratedCell[8];				//empty successors list to be returned
//        neighbors = getNeighborsPrune(GeneratedCell);    //all neighbors after pruned
//        for (int i=0; i<neighbors.length; i++){ //for each of these neighbors
//            tmpXY = jump(neighbors[i][0],neighbors[i][1],GeneratedCell.x,GeneratedCell.y); //get next jump point
//            if (tmpXY[0]!=-1){								//if that point is not null( {-1,-1} )
//                int x = tmpXY[0];
//                int y = tmpXY[1];
//                ng = (grid.toPointApprox(x,y,GeneratedCell.x,GeneratedCell.y) + GeneratedCell.g);   //get the distance from start
//                if (grid.getPair(x,y).f<=0 || grid.getPair(x,y).g>ng){  //if this GeneratedCell is not already found, or we have a shorter distance from the current GeneratedCell
//                    grid.getPair(x,y).updateGHFP(grid.toPointApprox(x,y,GeneratedCell.x,GeneratedCell.y)+GeneratedCell.g,grid.toPointApprox(x,y,endX,endY),GeneratedCell); //then update the rest of it
//                    successors[i] = grid.getPair(x,y);  //add this GeneratedCell to the successors list to be returned
//                }
//            }
//        }
...
//
//    /**
//     * jump method recursively searches in the direction of parent (px,py) to child, the current GeneratedCell (x,y).
//     * It will stop and return its current position in three situations:
//     *
//     * 1) The current GeneratedCell is the end GeneratedCell. (endX, endY)
//     * 2) The current GeneratedCell is a forced neighbor.
//     * 3) The current GeneratedCell is an intermediate step to a GeneratedCell that satisfies either 1) or 2)
//     *
//     * @param x (int) current GeneratedCell's x
//     * @param y (int) current GeneratedCell's y
//     * @param px (int) current.parent's x
//     * @param py (int) current.parent's y
//     * @return (int[]={x,y}) GeneratedCell which satisfies one of the conditions above, or null if no such GeneratedCell is found.
//     */
...
//
//    /**
//     * Returns Pairs that should be jumped based on the parent location in relation to the given GeneratedCell.
//     *
//     * @param GeneratedCell (GeneratedCell) GeneratedCell which has a parent (not the start GeneratedCell)
//     * @return (ArrayList<GeneratedCell>) list of Pairs that will be jumped
//     */
//    public int[][] getNeighborsPrune(GeneratedCell GeneratedCell){
//        GeneratedCell parent = GeneratedCell.parent;    //the parent GeneratedCell is retrieved for x,y values
//        int x = GeneratedCell.x;
//        int y = GeneratedCell.y;
//        int px, py, dx, dy;
...
//            return grid.getNeighbors(GeneratedCell); //adds initial Pairs to be jumped from!
//        }
//
