package exp.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.map.util;
...
//    //todo this occupiedPositions KeyListMap only accounts for initial positions. It is not like PositionTracker which has a system that updates. TO make it account for moving positions, do the same as in PositionTracker(Make a system and entities to have a certain component, and put them in teh biMap
    ...
    public static void initAvailableKeys(float mapWidth,float mapHeight){
        int capacity = exp.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.movement.PositionTracker.generateKey(mapWidth,mapHeight);//the last key
    ...
    public static Vector2[] generateRandomUCoordinates(int maxNumCoordinates){
        ...
        Rectangle mapBounds = exp.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.map.MapMaker.getMapBounds();
        int maxX = (int)(mapBounds.width);
        int maxY = (int)(mapBounds.height);

        Rectangle rectangle = new Rectangle();
        rectangle.width = exp.zhen.zayta.versions_unused.arcade_style_game.config.SizeManager.maxObjWidth;
        rectangle.height = exp.zhen.zayta.versions_unused.arcade_style_game.config.SizeManager.maxObjHeight;
...
//    }
    private static boolean withinBounds(Rectangle objBounds){
        return exp.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.map.MapMaker.getMapBounds().contains(objBounds);
    }
    ...
    public static Vector2[] generateCircleArrangementCoordinates(int numCoordinates){
        ...
        float minDim = Math.max(exp.zhen.zayta.versions_unused.arcade_style_game.config.SizeManager.maxObjWidth, SizeManager.maxObjHeight);
