...
public class GameFitViewport extends FitViewport {
    ...
    private void portraitView(int screenWidth, int screenHeight, boolean centerCamera){
        ...
        int gameHeight = (int)(screenHeight* exp.zhen.zayta.versions_unused.arcade_style_game.config.SizeManager.GAME_TO_CONTROL_RATIO);
        Vector2 scaled = getScaling().apply(getWorldWidth(), getWorldHeight(), screenWidth, gameHeight);
        int viewportWidth = Math.round(scaled.x);
        int viewportHeight = Math.round(scaled.y);

        // Center.
        setScreenBounds((screenWidth - viewportWidth) / 2,(gameHeight - viewportHeight) / 2+ (int)(exp.zhen.zayta.versions_unused.arcade_style_game.config.SizeManager.CONTROLLER_DIAMETER*screenHeight/ exp.zhen.zayta.versions_unused.arcade_style_game.config.SizeManager.HEIGHT), viewportWidth, viewportHeight);
        apply(centerCamera);
    }
    ...
    private void landscapeView(int screenWidth, int screenHeight, boolean centerCamera){
        ...
        int gameWidth = (int)(screenWidth* exp.zhen.zayta.versions_unused.arcade_style_game.config.SizeManager.GAME_TO_CONTROL_RATIO);
        Vector2 scaled = getScaling().apply(getWorldWidth(), getWorldHeight(), gameWidth, screenHeight);
        int viewportWidth = Math.round(scaled.x);
        int viewportHeight = Math.round(scaled.y);


        setScreenBounds((gameWidth - viewportWidth) / 2+ (int)(exp.zhen.zayta.versions_unused.arcade_style_game.config.SizeManager.CONTROLLER_DIAMETER*screenWidth/ SizeManager.WIDTH),(screenHeight - viewportHeight) / 2, viewportWidth, viewportHeight);
        apply(centerCamera);
    }
//
//    @Override
