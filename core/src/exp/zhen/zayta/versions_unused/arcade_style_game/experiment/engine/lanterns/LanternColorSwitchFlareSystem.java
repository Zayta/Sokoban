...
import exp.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.entity.id_tags.NighterTag;
...
public class LanternColorSwitchFlareSystem extends IteratingSystem implements Pool.Poolable {
    ...
    {
        super(Family.all(
                exp.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.entity.components.properties.ColorComponent.class,
                PositionTrackerComponent.class,
                RectangularBoundsComponent.class,
                HealthComponent.class
        ).one(NPCTag.class, NighterTag.class).exclude(exp.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.lanterns.LanternTag.class).get());
//        super(game,engine);
//        addMission();
        this.engine = engine;
...
//    }
    ...
    private void collideEvent(Entity entity, Entity lantern) {
        ...
        exp.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.entity.components.properties.ColorComponent entityColor = Mappers.COLOR.get(entity);
...
//        Mappers.COLOR.get(lantern).setColor(Mappers.COLOR.get(entity).getColor());
        exp.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.lanterns.LanternTag lanternTag = Mappers.LANTERN.get(lantern);
        ...
        if(lanternTag.getState()== exp.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.lanterns.LanternTag.State.DORMANT){
            if(lanternColor.getColor().equals(entityColor.getColor())){
                lanternTag.setState(exp.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.lanterns.LanternTag.State.FLARE);
...
//                particleAnimationComponent.init(RegionNames.FIRE_BLOB_PREFLARE,6,1);
        ...
        else if(lanternTag.getState()== exp.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.lanterns.LanternTag.State.FLARE){
            if(lanternColor.getColor().equals(entityColor.getColor())){
