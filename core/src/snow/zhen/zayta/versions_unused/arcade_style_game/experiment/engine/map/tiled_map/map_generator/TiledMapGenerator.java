...
public class TiledMapGenerator {
    ...
    public TiledMap generateWorld(snow.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.map.tiled_map.map_generator.MapType mapType){
        ...
        if(mapType!= snow.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.map.tiled_map.map_generator.MapType.NONE)
...
//        boolean [][] landOrWater = getLandOrWaterUnits(worldWidth,worldHeight);//true = land
    ...
    private TiledMapTileLayer visibleLayer(MapType mapType){
        ...
        visibleLayer.setName(snow.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.map.MapMaker.visibleLayer);
        for(int i = 0; i<worldWidth;i++){
            for(int j = 0; j<worldHeight;j++){
                TiledMapTileLayer.Cell cell = new TiledMapTileLayer.Cell();
                cell.setTile(getRandomTile(tileSet));
