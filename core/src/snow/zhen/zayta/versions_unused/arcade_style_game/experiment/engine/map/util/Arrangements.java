package snow.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.map.util;
...
//    }
    ...
    public static Vector2[] generateCircleArrangementCoordinates(int numCoordinates){
        float radius = 2.0f;
        Rectangle mapBounds = snow.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.map.MapMaker.getMapBounds();
        int maxX = (int)(mapBounds.width);
        int maxY = (int)(mapBounds.height);

        Circle circle = new Circle();
        float minDim = Math.max(SizeManager.maxObjWidth, snow.zhen.zayta.versions_unused.arcade_style_game.config.SizeManager.maxObjHeight);
