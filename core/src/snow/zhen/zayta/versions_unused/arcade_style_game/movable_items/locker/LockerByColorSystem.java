package snow.zhen.zayta.versions_unused.arcade_style_game.movable_items.locker;
...
public class LockerByColorSystem extends GameControllingSystem {
    ...
    private void initLockers(){
        ...
        for(int i =0; i<points.length; i++)
        {
            ...
            lockersKeyListMap.put(key,makeLocker(points[i].x,points[i].y, snow.zhen.zayta.versions_unused.arcade_style_game.movable_items.locker.LockerComponent.class/*,RegionNames.EMOTES_BLUE_EEK*/));
        }
//        //////log.debug("lockerKeyListMap: "+lockersKeyListMap);
    ...
    public void update(float deltaTime) {
        ...
        for(Entity lockerKey: lockerKeys) {
            int index = snow.zhen.zayta.versions_unused.arcade_style_game.experiment.common.Mappers.POSITION_TRACKER.get(lockerKey).getPositionKeyListMap().getKey(lockerKey);
            int indexAbove = index+ PositionTracker.n;
            int indexBelow = index- snow.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.movement.PositionTracker.n;
    ...
    {
        Color lockerColor = snow.zhen.zayta.versions_unused.arcade_style_game.experiment.common.Mappers.COLOR.get(locker).getColor();
        //////log.debug("LockerKey is " +lockerKey);
        Color lockerKeyColor = snow.zhen.zayta.versions_unused.arcade_style_game.experiment.common.Mappers.COLOR.get(lockerKey).getColor();
        return lockerColor==lockerKeyColor;
    }

    private boolean isInside(Entity locker, Entity lockerKey)
    {
        snow.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.movement.component.RectangularBoundsComponent lockerBounds = snow.zhen.zayta.versions_unused.arcade_style_game.experiment.common.Mappers.RECTANGULAR_BOUNDS.get(locker);
        snow.zhen.zayta.versions_unused.arcade_style_game.experiment.engine.movement.component.RectangularBoundsComponent lockerKeyBounds = snow.zhen.zayta.versions_unused.arcade_style_game.experiment.common.Mappers.RECTANGULAR_BOUNDS.get(lockerKey);
