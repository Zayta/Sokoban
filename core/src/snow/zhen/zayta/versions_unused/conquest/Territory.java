package snow.zhen.zayta.versions_unused.conquest;
...
import snow.zhen.zayta.versions_unused.conquest.soldiers.nur.NUR;
import snow.zhen.zayta.versions_unused.conquest.soldiers.utsubyo.Utsubyo;
import snow.zhen.zayta.versions_unused.arcade_style_game.characters.nur.Undead;
import snow.zhen.zayta.versions_unused.conquest.tiles.Tile;
...
//        menuButton();
    ...
    public snow.zhen.zayta.versions_unused.conquest.tiles.Tile[][] initMPos(snow.zhen.zayta.versions_unused.conquest.tiles.Tile[] nPos){
        snow.zhen.zayta.versions_unused.conquest.tiles.Tile[] [] mPos = new snow.zhen.zayta.versions_unused.conquest.tiles.Tile[numRows][numColumns];
        for(int i = 0; i< numRows; i++){
            for(int j = 0; j<numColumns; j++){
                float left = padding + (j+1)*(tileWidth+padding), bottom = i*(tileHeight+padding)+padding/2;
                snow.zhen.zayta.versions_unused.conquest.tiles.Tile mpos = new snow.zhen.zayta.versions_unused.conquest.tiles.Tile(conquestAtlas.findRegion("backgrounds/sunrise_parallax"),Utsubyo.generateMonster(j));
    ...
    public snow.zhen.zayta.versions_unused.conquest.tiles.Tile[] initNPos(NUR nur){
        snow.zhen.zayta.versions_unused.conquest.tiles.Tile[] nPos = new snow.zhen.zayta.versions_unused.conquest.tiles.Tile[numRows];
        for(int i = 0; i<numRows;i++){
            float left = padding, bottom = i*(tileHeight+padding)+padding/2;
            snow.zhen.zayta.versions_unused.conquest.tiles.Tile npos = new snow.zhen.zayta.versions_unused.conquest.tiles.Tile(conquestAtlas.findRegion("backgrounds/day sky"),nur.summon(Undead.Lorale));
            nPos[i]= npos;
